...
describe("dispatchTool", () => {
  it("update_profile writes the name to the store", async () => {
    ...
    await dispatchTool("update_profile", { field: "name", value: "Harsh" }, { phone: "+1", store, actions: stubActions });
  ...
  it("update_profile saves an emergency contact", async () => {
    ...
    await dispatchTool(
      ...
      { phone: "+1", store, actions: stubActions },
    );
  ...
  it("block_intercept denies a blocklisted name", async () => {
    ...
    const raw = await dispatchTool("block_intercept", { target_name: "Jordan" }, { phone: "+1", store, actions: stubActions });
    expect(JSON.parse(raw).allow).toBe(false);
  });

  it("set_party_mode arms the night", async () => {
    const store = new MemoryStore();
    await dispatchTool("set_party_mode", { active: true }, { phone: "+1", store, actions: stubActions });
